...
describe('StakingManager', () => {
  ...
  const MIN = 1_000_000; // 1 STX (minStake)
  ...
  describe('unstake', () => {
    ...
    test('throws if insufficient staked balance', () => {
      sm.stake('alice', MIN);
      (sm as any).stakeTimestamps.set('alice', Date.now() - sm.lockPeriod - 1);
    ...
    test('removes user from maps when fully unstaked', () => {
      ...
      expect((sm as any).stakes.has('alice')).toBe(false);
    });
  });
  ...
  describe('calculateRewards', () => {
    ...
    test('returns stake * rewardRate per epoch', () => {
      sm.stake('alice', MIN);
    ...
    test('reward scales linearly with number of epochs', () => {
      sm.stake('alice', MIN);
      (sm as any).stakeTimestamps.set('alice', Date.now() - sm.lockPeriod * 5 - 1);
      const rewards = sm.calculateRewards('alice');
